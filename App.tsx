...
const App: React.FC = () => {
    ...
    return (
        <div className="h-screen bg-gray-50 flex flex-col md:flex-row font-sans text-gray-900 overflow-hidden">
            ...
            {/* Mobile Header */}
            <div className="md:hidden bg-white border-b border-gray-200 p-4 flex justify-between items-center sticky top-0 z-20 flex-shrink-0">
      ...
      `}>
                <div className="p-6 pb-0 flex-shrink-0 bg-white z-10">
                    <h1 className="hidden md:flex items-center gap-2 text-2xl font-serif font-bold text-mystic-800 mb-8 select-none">
                ...
                <div className="flex-1 overflow-y-auto px-6 pb-6 pt-2 scrollbar-thin scrollbar-thumb-gray-200">
                    <div className="space-y-1">
                        <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 sticky top-0 bg-white py-1 z-10">我的档案</h3>
                        {profiles.map(p => (
                            <button
                                key={p.id}
                                onClick={() => { setActiveProfileId(p.id); Storage.setActiveProfileId(p.id); setIsSidebarOpen(false); }}
                                className={`w-full text-left px-3 py-2 rounded-md text-sm flex justify-between items-center group transition-colors ${activeProfileId === p.id ? 'bg-mystic-50 text-mystic-700 font-medium' : 'text-gray-600 hover:bg-gray-50'}`}
                            >
                                <span className="flex items-center gap-2 truncate">
                                    <User size={14} className="flex-shrink-0" /> <span className="truncate">{p.name}</span>
                                </span>
                                {activeProfileId === p.id && (
                                    <Trash2 size={12} className="opacity-50 hover:opacity-100 hover:text-red-500 flex-shrink-0" onClick={(e) => { e.stopPropagation(); handleDeleteProfile(p.id); }} />
                                )}
                            </button>
                        ))}
                    </div>
